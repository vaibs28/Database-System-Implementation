...
SelectFile SFps, SF_p, SF_s, SF_o, SF_li, SF_c;
DBFile dbfPS, dbf_p, dbf_s, dbf_o, dbf_li, dbf_c;
...
void q1() {
    ...
    SFps.Run(dbfPS, _ps, cnf_ps, lit_ps);
    SFps.WaitUntilDone();
...
void q4() {
    ...
    SFps.Run(dbfPS, _ps, cnf_ps, lit_ps); // 161 recs qualified
...
void q6() {
    ...
    OrderMaker grp_order2;
    grp_order2.Add(3,Int);
    G.Use_n_Pages(1);

    SFps.Run(dbfPS, _ps, cnf_ps, lit_ps); // 161 recs qualified
    J.Use_n_Pages(1);
    J.Run(_s, _ps, _s_ps, cnf_p_ps, lit_p_ps);
    G.Run(_s_ps, _out, grp_order2, func);
